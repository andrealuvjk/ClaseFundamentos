...
def estudiantesMatriculados(ListaDeEstudiantes):
    ...
    j = 0
    matricula = []
    for revision in ListaDeEstudiantes:
        definirM = revision[j][3]
        if definirM == "registrado":
            nombres.append(TercerGrado[j][2])
        else:
            pass  # un comodin para que la funcion no falle apesar de no tener logica de ejecuion
        j += 1
    print(matricula)


estudiantesMatriculados(TercerGrado)
